...
export const SettingsModule: React.FC<SettingsModuleProps> = ({ onShowToast }) => {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'jira' | 'appearance'>('users');
  ...
  return (
    <div className="space-y-6">
      {/* Header Bar */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-5 bg-white dark:bg-[#0d1424] border border-slate-200 dark:border-slate-800 rounded-3xl shadow-xl transition-colors">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-slate-700 via-indigo-600 to-blue-600 flex items-center justify-center shadow-lg shadow-indigo-600/30 ring-1 ring-black/5 dark:ring-white/20">
            <Settings className="w-6 h-6 text-white" />
          </div>
          <div>
            <div className="flex items-center gap-2.5">
              <h2 className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">Painel de Configurações</h2>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2.5 px-3.5 py-2 rounded-2xl bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-medium shadow-inner">
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-slate-700 dark:text-slate-300 font-semibold">PostgreSQL</span>
              <span className="text-emerald-600 dark:text-emerald-400 font-bold">Conectado</span>
            </div>
          </div>

          {/* Tab Switcher */}
          <div className="flex flex-wrap items-center gap-1.5 p-1.5 bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-inner w-full sm:w-auto">
            <button
      ...
      {activeTab === 'users' && (
        <div className="space-y-4">
          {/* Controls Bar */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3.5 bg-white dark:bg-[#0c1222] border border-slate-200 dark:border-slate-800 rounded-2xl transition-colors">
            <div className="relative flex-1 w-full sm:max-w-md">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500"
              />
            </div>
          ...
          {/* Tabela de Usuários */}
          <div className="bg-white dark:bg-[#0c1222] border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-xl transition-colors">
            {isLoadingUsers ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-950/80 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                          ...
                          <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                            ...
                            <td className="py-3.5 px-5">
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-indigo-600/40 to-violet-600/40 text-indigo-600 dark:text-indigo-300 font-bold flex items-center justify-center text-xs border border-indigo-500/30">
                                  {u.username.charAt(0).toUpperCase()}
                                </div>
                                <div>
                                  <div className="flex items-center gap-1.5">
                                    <span className="font-bold text-slate-900 dark:text-white text-sm">{u.username}</span>
                                    {u.isAdmin && (
                                      <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-500/20">
                                        ADMIN
                                      </span>
                                    )}
                                  </div>
                            ...
                            <td className="py-3.5 px-4">
                              <div className="flex flex-wrap items-center gap-1.5">
                                {(hasTasks || u.isAdmin) && (
                                  <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-indigo-500/10 dark:bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30">
                                    Tarefas
                                  </span>
                                )}
                                {(hasCards || u.isAdmin) && (
                                  <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-purple-500/10 dark:bg-purple-500/15 text-purple-700 dark:text-purple-300 border border-purple-500/30">
                                    Cards
                                  </span>
                                )}
                                {(hasHealth || u.isAdmin) && (
                                  <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-emerald-500/10 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30">
                                    Saúde & Peso
                                  </span>
                                )}
                                {(hasDashboards || u.isAdmin) && (
                                  <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-cyan-500/10 dark:bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 border border-cyan-500/30">
                                    Dashboards
                                  </span>
                                )}
                                {(u.canAccessJira || u.isAdmin) && (
                                  <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-blue-500/10 dark:bg-blue-500/15 text-blue-700 dark:text-blue-300 border border-blue-500/30">
                                    Jira
                                  </span>
                                )}
                                {!hasTasks && !hasCards && !hasHealth && !hasDashboards && !u.canAccessJira && !u.isAdmin && (
                                  <span className="text-[11px] text-slate-400 dark:text-slate-500 italic">Nenhum</span>
                                )}
                              </div>
                            </td>
                            ...
                            <td className="py-3.5 px-4">
                              {u.isDefaultPassword ? (
                                <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-amber-700 dark:text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2.5 py-0.5 rounded-lg">
                                  <AlertCircle className="w-3 h-3" />
                                  Troca Obrigatória
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-0.5 rounded-lg">
      ...
      {/* ========================================================== */}
      {activeTab === 'jira' && (
        <div className="bg-white dark:bg-[#0c1222] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xl space-y-6 transition-colors">
          <div className="flex items-start justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <svg className="w-4 h-4 fill-blue-500 dark:fill-blue-400" viewBox="0 0 24 24">
                  <path d="M11.53 2c0 2.4 1.97 4.35 4.38 4.35h2.15v2.17c0 2.4 1.97 4.35 4.39 4.35V2h-10.92zm-5.77 5.79c0 2.4 1.97 4.35 4.39 4.35h2.14v2.17c0 2.4 1.97 4.35 4.39 4.35V7.79H5.76zm-5.76 5.79c0 2.4 1.97 4.35 4.39 4.35h2.14v2.17c0 2.4 1.97 4.35 4.39 4.35v-10.87H0z"/>
                </svg>
          ...
          <form onSubmit={handleSaveJiraSettings} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 font-mono"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-[11px] text-indigo-500 dark:text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 flex items-center gap-1"
                >
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-3 pr-10 py-2 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 font-mono"
                />
                <button
                  type="button"
            ...
            <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-slate-800">
              ...
              <div className="flex items-center gap-2 max-w-md">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white uppercase font-mono"
                />
                <button
                  type="button"
                  onClick={handleAddProject}
                  className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-white text-xs font-semibold"
                >
                  Adicionar
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {projects.map((p) => (
                  <span
                    key={p}
                    className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30 text-xs font-mono font-bold"
                  >
                    <span>{p}</span>
                    <button
                      type="button"
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-[11px]">
                  ...
                  <button
                    ...
                    className="text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 font-semibold"
                  >
                    Desmarcar Todos
                  </button>
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 p-2 rounded-xl text-xs text-left border transition-all ${
                        isChecked
                          ? 'bg-indigo-50 dark:bg-indigo-600/20 text-indigo-700 dark:text-indigo-200 border-indigo-300 dark:border-indigo-500/40 font-semibold'
                    ...
                    >
                      {isChecked ? (
                        ...
                        <Square className="w-3.5 h-3.5 text-slate-400 dark:text-slate-600 flex-shrink-0" />
                      )}
            ...
            <div className="space-y-3 pt-3 border-t border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[11px] text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-semibold transition-colors"
              ...
              <div className="flex items-center gap-2 max-w-md">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white font-mono placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                />
                <button
                  type="button"
                  onClick={handleAddIgnoredField}
                  className="px-3.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-white text-xs font-semibold transition-all"
                >
                  Adicionar
                </button>
              </div>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {ignoredFields.length === 0 ? (
                  ...
                  ignoredFields.map((field) => (
                    <span
                      key={field}
                      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-slate-100 dark:bg-slate-800/90 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 text-xs font-medium shadow-sm hover:border-slate-400 dark:hover:border-slate-600 transition-colors"
                    >
                      <span>{field}</span>
                      <button
                        type="button"
                        onClick={() => handleRemoveIgnoredField(field)}
                        className="text-slate-400 hover:text-rose-500 text-sm font-bold ml-1 transition-colors"
                        title={`Remover ${field}`}
                      >
                        ×
                      </button>
            ...
            {testResult && (
              <div
                className={`p-4 rounded-2xl text-xs flex items-start gap-2.5 border ${
                  testResult.success
                    ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-800 dark:text-emerald-300 border-emerald-200 dark:border-emerald-500/30'
                    : 'bg-rose-50 dark:bg-rose-950/30 text-rose-800 dark:text-rose-300 border-rose-200 dark:border-rose-500/30'
                }`}
              >
                {testResult.success ? (
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex flex-wrap items-center justify-between gap-3">
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 disabled:opacity-50 text-slate-700 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white border border-slate-300 dark:border-slate-700 text-xs font-semibold transition-all"
              >
                {isTestingJira ? (
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-500 dark:text-indigo-400" />
                ) : (
                  <RotateCw className="w-3.5 h-3.5 text-indigo-500 dark:text-indigo-400" />
                )}
        ...
        <div className="bg-white dark:bg-[#0c1222] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-xl space-y-6 transition-colors">
          <div className="flex items-start justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                Personalização de Tema & Aparência
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Escolha o tema visual de sua preferência ou sincronize automaticamente com o sistema operacional.
              </p>
            </div>
            <span className="px-3 py-1 rounded-xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 text-xs font-bold flex items-center gap-1.5">
              Tema ativo: {resolvedTheme === 'dark' ? 'Modo Escuro' : 'Modo Claro'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <button
              ...
              onClick={() => {
                setTheme('light');
                onShowToast('Tema Claro ativado.', 'info');
              }}
            ...
            >
              ...
              <div>
                <h4 className="font-extrabold text-sm text-slate-900 dark:text-white">Tema Claro</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Fundo limpo e iluminado com alto contraste de texto e cores saturadas.
                </p>
              </div>
            </button>
            ...
            <button
              ...
              onClick={() => {
                setTheme('dark');
                onShowToast('Tema Escuro ativado.', 'info');
              }}
              className={`p-5 rounded-2xl border text-left flex flex-col justify-between gap-4 transition-all ${
                theme === 'dark'
            ...
            >
              ...
              <div>
                <h4 className="font-extrabold text-sm text-slate-900 dark:text-white">Tema Escuro</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Design noturno profundo e confortável para os olhos em ambientes de baixa iluminação.
                </p>
              </div>
            </button>
            ...
            <button
              ...
              onClick={() => {
                setTheme('system');
                onShowToast('Tema Automático (Sistema) ativado.', 'info');
              }}
              className={`p-5 rounded-2xl border text-left flex flex-col justify-between gap-4 transition-all ${
                theme === 'system'
            ...
            >
              ...
              <div>
                <h4 className="font-extrabold text-sm text-slate-900 dark:text-white">Automático (Sistema)</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Acompanha automaticamente as configurações de modo claro ou escuro do seu dispositivo.
                </p>
              </div>
            </button>
          </div>
      ...
      {isCreateUserOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#0b101e] border border-slate-200 dark:border-slate-700 rounded-3xl max-w-md w-full shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 transition-colors">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2">
                <UserPlus className="w-4 h-4 text-indigo-500 dark:text-indigo-400" />
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">Cadastrar Novo Usuário</h3>
              </div>
              <button
            ...
            <form onSubmit={handleCreateUserSubmit} className="p-6 space-y-4">
              {createUserError && (
                <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-500/30 text-rose-600 dark:text-rose-300 text-xs">
                  {createUserError}
                </div>
              )}

              <div className="space-y-1">
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Login / Username: *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                />
              </div>

              <div className="space-y-1">
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Senha Inicial Temporária: *</label>
                <input
                  type="password"
                  required
              ...
              <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-xs cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors">
                    <input
                      type="checkbox"
                    ...
                    />
                    <div>
                      <span className="font-bold text-slate-900 dark:text-white block">📋 Gestão de Tarefas</span>
                  ...
                  <label className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-xs cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors">
                    ...
                    />
                    <div>
                      <span className="font-bold text-slate-900 dark:text-white block">✍️ Escrita de Cards</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
                  ...
                  <label className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-xs cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors">
                    <input
                      ...
                      className="rounded bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-700 text-emerald-600 focus:ring-0"
                    />
                    <div>
                      <span className="font-bold text-slate-900 dark:text-white block">⚖️ Saúde & Peso</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
                  ...
                  <label className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-xs cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors">
                    <input
                      ...
                      className="rounded bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-700 text-cyan-600 focus:ring-0"
                    />
                    <div>
                      <span className="font-bold text-slate-900 dark:text-white block">📊 Dashboards Analíticos</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
                  ...
                  <label className="flex items-center gap-2.5 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-xs cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors">
                    <input
                      ...
                      className="rounded bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-0"
                    />
                    <div>
                      <span className="font-bold text-slate-900 dark:text-white block">🔷 Demandas Jira</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
              ...
              <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold shadow-md shadow-indigo-600/30 transition-all"
                >
      ...
      {manageModalUser && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-[#0b101e] border border-slate-200 dark:border-slate-700 rounded-3xl max-w-xl w-full shadow-2xl p-6 space-y-6 animate-in fade-in zoom-in-95 my-8 transition-colors">
            ...
            <div className="flex items-start justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-extrabold text-slate-900 dark:text-white">{manageModalUser.username}</h3>
                    {manageModalUser.isAdmin && (
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-amber-500/10 text-amber-600 dark:text-amber-300 border border-amber-500/20">
                        ADMINISTRADOR
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Gerenciamento de módulos, permissões e credenciais</p>
                </div>
              </div>
              <button
                type="button"
            ...
            {manageError && (
              <div className="p-3 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-500/30 text-rose-600 dark:text-rose-300 text-xs flex items-center gap-2">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400">Defina os módulos que este usuário tem autorização para utilizar:</p>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <label className={`p-3 rounded-2xl border flex items-start gap-3 cursor-pointer transition-all ${
                    manageAllowedModules.includes('tasks')
                      ? 'bg-indigo-50 dark:bg-indigo-600/10 border-indigo-200 dark:border-indigo-500/40 text-slate-900 dark:text-white'
                    ...
                    <div className="flex-1">
                      <span className="font-bold text-xs block text-slate-900 dark:text-slate-200">Gestão de Tarefas</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
                  ...
                  <label className={`p-3 rounded-2xl border flex items-start gap-3 cursor-pointer transition-all ${
                    manageAllowedModules.includes('cards')
                      ? 'bg-purple-50 dark:bg-purple-600/10 border-purple-200 dark:border-purple-500/40 text-slate-900 dark:text-white'
                      : 'bg-slate-50 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                  }`}>
                    <input
                      type="checkbox"
                      ...
                      className="mt-0.5 rounded border-slate-300 dark:border-slate-700 text-purple-600 focus:ring-purple-500 w-4 h-4"
                    />
                    <div className="flex-1">
                      <span className="font-bold text-xs block text-slate-900 dark:text-slate-200">Escrita de Cards</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
                  ...
                  <label className={`p-3 rounded-2xl border flex items-start gap-3 cursor-pointer transition-all ${
                    manageAllowedModules.includes('health')
                      ? 'bg-emerald-50 dark:bg-emerald-600/10 border-emerald-200 dark:border-emerald-500/40 text-slate-900 dark:text-white'
                      : 'bg-slate-50 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                  }`}>
                    <input
                      type="checkbox"
                    ...
                    />
                    <div className="flex-1">
                      <span className="font-bold text-xs block text-slate-900 dark:text-slate-200">Saúde & Evolução de Peso</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
                  ...
                  <label className={`p-3 rounded-2xl border flex items-start gap-3 cursor-pointer transition-all ${
                    manageAllowedModules.includes('dashboards')
                      ? 'bg-cyan-50 dark:bg-cyan-600/10 border-cyan-200 dark:border-cyan-500/40 text-slate-900 dark:text-white'
                      : 'bg-slate-50 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                  }`}>
                    <input
                      type="checkbox"
                      ...
                      className="mt-0.5 rounded border-slate-300 dark:border-slate-700 text-cyan-600 focus:ring-cyan-500 w-4 h-4"
                    />
                    <div className="flex-1">
                      <span className="font-bold text-xs block text-slate-900 dark:text-slate-200">Dashboards Analíticos</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
                  ...
                  <label className={`p-3 rounded-2xl border flex items-start gap-3 cursor-pointer transition-all ${
                    manageCanAccessJira
                      ? 'bg-blue-50 dark:bg-blue-600/10 border-blue-200 dark:border-blue-500/40 text-slate-900 dark:text-white'
                      : 'bg-slate-50 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                  }`}>
                    <input
                      type="checkbox"
                      ...
                      className="mt-0.5 rounded border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-blue-500 w-4 h-4"
                    />
                    <div className="flex-1">
                      <span className="font-bold text-xs block text-slate-900 dark:text-slate-200">Acesso ao Painel do Jira</span>
                      <span className="text-[11px] text-slate-500 dark:text-slate-400 block">
            ...
            <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800">
              ...
              {managePasswordError && (
                <div className="p-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-500/30 text-rose-600 dark:text-rose-300 text-xs">
                  {managePasswordError}
                </div>
              )}

              {managePasswordSuccess && (
                <div className="p-2.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-500/30 text-emerald-600 dark:text-emerald-300 text-xs flex items-center gap-1.5">
              ...
              <form onSubmit={handleResetPasswordInModal} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-amber-500"
                />
                <button
                  type="submit"
              ...
              <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-slate-800">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl bg-rose-50 dark:bg-rose-500/10 hover:bg-rose-100 dark:hover:bg-rose-500/20 border border-rose-200 dark:border-rose-500/30 hover:border-rose-300 dark:hover:border-rose-500/50 text-rose-600 dark:text-rose-300 hover:text-rose-700 dark:hover:text-rose-200 text-xs font-bold transition-all flex items-center justify-center gap-1.5 flex-shrink-0"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-end">
              <button
                type="button"
                onClick={() => setManageModalUser(null)}
                className="px-5 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-all"
              >
                Fechar
              </button>
            </div>
